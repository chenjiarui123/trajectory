...
def get_ESM_points_xy(ESM_points, sensors):
    ...
    for group, data in tqdm(grouped_ESM_points, desc='交叉定位ESM'):
        if data.shape[0] < 2:
            ...
            if check_distance(point_xy, sensor_xy1) and check_distance(point_xy, sensor_xy2):
...
def dedumplacate(radar_points_file, located_ESM_points_file, deduplacate_radar_file, safe_distance=200):
    ...
    retain_points = []
    deduplacate_points = []
    for time, data in tqdm(located_points_group, desc='雷达点去重'):
...
def merge_unlocated_ESM_with_rader(unlocated_ESM_file, radar_file, located_ESM_file, sensor_coords_file, radar_file_updated, located_ESM_updated):
    ...
    located =  []
    #remove_indexes = []
    for i, row in tqdm(unlocated_ESM.iterrows(), total=len(unlocated_ESM), desc='匹配未定位ESM'):
